...
from ..tools.sms_api_evolution import SmsApiEvolution
...
class SmsSms(models.Model):
    ...
    def _send(self, unlink_failed=False, unlink_sent=True, raise_exception=False):
        """Route sending via Evolution when the IAP account provider is 'evolution'.

        Falls back to the standard implementation otherwise.
        """
        # Determine if current database is configured to use Evolution for SMS
        ...
        # Build messages in the same structure as base implementation
        messages = [{
            'content': body,
        ...
        try:
            results = SmsApiEvolution(self.env, account=account)._send_sms_batch(messages, delivery_reports_url=delivery_reports_url)
        except Exception as e:
            _logger.info('Evolution send batch %s SMS: %s: failed with exception %s', len(self.ids), self.ids, e)
            if raise_exception:
                raise
            results = [{'uuid': sms.uuid, 'state': 'server_error'} for sms in self]

        # Reuse the base processing logic for results mapping and write operations
